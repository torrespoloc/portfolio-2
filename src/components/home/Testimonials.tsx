...
function Avatar({ name, image, light }: { name: string; image?: string; light?: boolean }) {
 ...
 return (
 <div
 className={`flex h-[72px] w-[72px] rounded-full shrink-0 items-center justify-center ${
...
export function Testimonials({ className = "" }: Props) {
 ...
 <div className="grid gap-0 border-t border-hero-border/60 lg:grid-cols-[minmax(0,1.45fr)_minmax(0,0.75fr)]">
 <div className="grid gap-0 border-r border-hero-border/60">
 <motion.article
 variants={fadeUp}
 transition={{ duration: 0.55, ease: [0.22, 1, 0.36, 1] }}
 className="relative flex h-full flex-col overflow-hidden rounded-none border-b border-hero-border/60 bg-burgundy p-8 text-white"
 >
 ...
 className={`flex h-full flex-col rounded-none border-b border-hero-border/60 ${index === 0 ? "border-r border-hero-border/60" : ""} bg-white/85 dark:bg-hero-card p-8 transition-transform duration-300 hover:-translate-y-1`}
 >
 ...
 <article
 key={testimonial.name}
 className={`flex h-full flex-col rounded-none border-b border-hero-border/60 ${index === 0 ? "border-b border-hero-border/60 sm:border-r lg:border-r-0 lg:border-b" : ""} bg-white/85 dark:bg-hero-card p-8 transition-transform duration-300 hover:-translate-y-1`}
 >
 <p className="flex-1 text-sm sm:text-body-lg leading-[1.65] text-hero-text-dark">
 &ldquo;{highlightText(testimonial.quote, testimonial.highlights ?? [])}&rdquo;
 </p>
