...
export function WorkCardsStack({ show7dish = true }: WorkCardsStackProps) {
  ...
  return (
    <MotionConfig reducedMotion="user">
      ...
      <section id="work" className="relative border-t border-hero-border/70">
          ...
          >
            {visibleWorkCards.flatMap((card, index) => {
              ...
              if (isFirstInRow && !isLast) {
                elements.push(
                  <div
                    ...
                    className="hidden lg:block border-x border-hero-border/70 w-6 self-stretch"
                  />
                )
              }
              ...
              elements.push(
                <div
                  key={`sep-${card.href}`}
                  className={`border-y border-hero-border/70 h-4 ${
                    isRowEnd
